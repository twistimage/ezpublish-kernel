...
class DoctrineDatabase extends Gateway {
    ...
    public function find(Criterion $criterion, $offset = 0, $limit = null, array $sortClauses = null, $doCount = true) {
        ...
        $selectQuery
                ...
                ->innerJoin(
                        'ezcontentobject', 'ezcontentobject_tree.contentobject_id', 'ezcontentobject.id'
                )
                ->innerJoin(
                        'ezcontentobject_version', 'ezcontentobject.id', 'ezcontentobject_version.contentobject_id'
        );
        if ($sortClauses !== null) {
            $this->sortClauseConverter->applyJoin($selectQuery, $sortClauses, $fieldMap);
        }
        $selectQuery->where(
                $this->criteriaConverter->convertCriteria($selectQuery, $criterion), $selectQuery->expr->eq(
                        ...
                        $this->handler->quoteColumn("depth", "ezcontentobject_tree"), $selectQuery->bindValue(0, null, PDO::PARAM_INT)
                )
        );
        if ($sortClauses !== null) {
        ...
        $selectQuery->limit(
                $limit > 0 ? $limit : self::MAX_LIMIT, $offset
        );
    ...
    protected function getTotalCount(Criterion $criterion, $sortClauses) {
        ...
        $query->where(
                $this->criteriaConverter->convertCriteria($query, $criterion), $query->expr->eq(
                        'ezcontentobject.status',
                        //ContentInfo::STATUS_PUBLISHED
                        ...
                        $this->handler->quoteColumn("depth", "ezcontentobject_tree"), $query->bindValue(0, null, PDO::PARAM_INT)
                )
        );
